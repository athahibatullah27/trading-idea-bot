...
export async function generateDerivativesTradeIdea(
  ...
  context?: string
): Promise<GeneratedTradeIdeaResult> {
  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      ...
      endPerformanceTimer(timerId);
      return { tradeIdea: null, modelUsed: null };
    }
    ...
    if (error.message && (error.message.includes('429') || error.message.includes('Too Many Requests') || error.message.includes('quota'))) {
      ...
      try {
        modelUsed = OPENROUTER_GEMINI_MODEL;
        ...
        if (fallbackTradeIdea) {
          ...
          endPerformanceTimer(timerId);
          return { tradeIdea: fallbackTradeIdea, modelUsed };
        }
      } catch (fallbackError) {
      ...
      return { tradeIdea: quotaExceededIdea, modelUsed: null };
    }
...
- ActivatedSignals must be an array of strings indicating which signals contributed to the analysis. Valid signal names are: "multi_timeframe_trend_alignment", "volume_confirmation", "market_regime_consistency", "fibonacci_confluence", "support_resistance_reaction", "momentum_alignment", "bollinger_band_position", "ema_alignment", "candlestick_patterns"
...
function parseDerivativesTradeResponse(text: string, marketData: EnhancedDerivativesMarketData): DerivativesTradeIdea | null {
  ...
  try {
    ...
    // Extract activated signals for signal tracking (if provided by Gemini)
